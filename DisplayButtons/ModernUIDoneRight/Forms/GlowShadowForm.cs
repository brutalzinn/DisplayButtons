...
namespace NickAc.ModernUIDoneRight.Forms
{
    ...
    /// </summary>
    public class GlowShadowForm : Form
    {
        ...
        public static Bitmap DrawShadowBitmap(int width, int height, int borderRadius, int blur, int spread,
            Color color)
        {
            int ex = blur + spread;
            int w = width + ex * 2;
            ...
            if (blur > 0)
            {
                // four dir gradiant
                {
                    ...
                    g.FillRectangle(brush, blur, blur, blur, solidH);
                    // up
